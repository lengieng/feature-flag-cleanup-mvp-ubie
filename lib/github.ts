...
export async function createCleanupIssue(flag: Flag) {
  ...
  const title = `🧹 Cleanup feature flag: ${flag.key}`;

  const body = `## 🤖 Automated Feature Flag Cleanup
...
### ⚠️ Why This Flag Should Be Removed

${analysis.reason}

### 🛠️ Cleanup Instructions for @copilot
...
**⏰ Lifecycle:** This issue will be automatically closed when the cleanup PR is merged or after 7 days if no action is taken.

**🏷️ Labels:** This issue is labeled \`automated-cleanup\` and can be filtered from your main issue view using: \`is:issue label:automated-cleanup\`

**📊 Dashboard:** Track cleanup status at your dashboard

---

Generated by **Feature Flag Cleanup MVP** • *Making feature flag technical debt a thing of the past*
`;
